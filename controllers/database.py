...
class BancoDeDados:
    ...
    def verificar_senha(usuario_id: int, senha: str) -> bool:
        """
        LÓGICA BOOLEANA: confirma se a senha informada corresponde à do usuário.
        Usada como camada de segurança antes de exclusões/ações sensíveis.
        Retorna True se (hash(senha) == senha_armazenada).
        """
        conn = BancoDeDados._conectar()
        c = conn.cursor()
        c.execute("SELECT senha FROM usuarios WHERE id = ?", (usuario_id,))
        row = c.fetchone()
        conn.close()
        if not row:
            return False
        return row[0] == BancoDeDados._hash_senha(senha)

    @staticmethod
    def excluir_conta(usuario_id: int, senha: str) -> bool:
        """
        DELETE (CRUD) — Exclui PERMANENTEMENTE a conta do jogador.

        Remove o usuário e TODOS os seus dados relacionados (partidas e
        conquistas), garantindo que ele desapareça de todos os rankings.
        A exclusão só ocorre se a senha for confirmada (proposição booleana).

        Retorna True se a exclusão foi concluída; False se a senha estiver
        incorreta.
        """
        # LÓGICA BOOLEANA: só prossegue se a senha conferir
        if not BancoDeDados.verificar_senha(usuario_id, senha):
            return False

        conn = BancoDeDados._conectar()
        c = conn.cursor()
        # Remove dados relacionados primeiro (integridade referencial),
        # depois o próprio usuário. Assim seu nome some dos rankings.
        c.execute("DELETE FROM partidas   WHERE usuario_id = ?", (usuario_id,))
        c.execute("DELETE FROM conquistas WHERE usuario_id = ?", (usuario_id,))
